...
import type { LuzpressSocialService } from "../docs/socials";

export type { LuzpressSocialService } from "../docs/socials";
...
} as const satisfies Record<LuzpressSocialService, string>;

/** Social brand icon — `icon` matches `luzpress({ socials })` `service` values. */
export function Icon(props: { icon: LuzpressSocialService; class?: string }): RawHtml {
  return svg(props.class, PATHS[props.icon]);
}

/** @deprecated Use `<Icon icon="github" />` from `luzpress/icons`. */
