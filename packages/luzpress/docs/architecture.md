...
| Components | `src/components/`  | Ilha, Areia, virtual `luzpress/config`, `luzpress/mdx`, `luzpress/runtime` |
...
Runtime virtual modules (`luzpress/mdx`, `luzpress/config`, `luzpress/shiki`, `luzpress/landing-shiki`) are generated or transformed by the `luzpress:config` Vite plugin in dev/build; stubs exist under `src/docs/` for types and npm resolution.
