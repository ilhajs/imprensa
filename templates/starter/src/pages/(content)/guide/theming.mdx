---
title: Theming
description: Customize Luz with Areia tokens, dark mode, fonts, logos, and Shiki code themes.
order: 4
tags: [theme, areia]
---
...
Luz UI is built with [Areia](https://areia.ilha.build) and Tailwind CSS v4. The starter keeps styling mostly in utility classes and theme tokens, so you can rebrand without rewriting components.
...
The choice is saved in `localStorage` as `luz:theme`. The runtime applies the theme before rendering to avoid a flash of the wrong color scheme.

## Logo and brand name

Replace `public/logo.svg` with your logo. The default `LogoButton` from `luzpress/components` renders `/logo.svg` beside the text “Luz”.
